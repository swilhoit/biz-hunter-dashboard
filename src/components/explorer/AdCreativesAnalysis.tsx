...
import { Eye, Video, Image, TrendingUp, AlertCircle, Search, X } from 'lucide-react';
import { fetchAdCreatives } from '../../utils/explorer/rainforestApi';
...
export function AdCreativesAnalysis({ keywords, products }: AdCreativesAnalysisProps) {
  ...
  const handleSearchAds = useCallback(async () => {
    if (!searchKeyword.trim()) {
      setError('Please enter a keyword to search for ads');
      return;
    }
    ...
    try {
      const creatives = await fetchAdCreatives(searchKeyword);
      setAdCreatives(creatives);
    } catch (err) {
      setError('Failed to fetch ad creatives. Please check your API settings.');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-100">Amazon Ad Creatives Analysis</h3>
      ...
      {sortedBrands.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <h4 className="text-md font-semibold mb-4 text-gray-800 dark:text-gray-100">Top Brands in Your Search</h4>
                ...
                <div className="relative aspect-video bg-gray-100 dark:bg-gray-700">
                  {creative.type === 'image' ? (
                    <img
                      ...
                      onError={(e) => {
                        e.target.src = 'https://via.placeholder.com/300x200?text=Ad+Creative';
                      }}
                    />
                  ) : (
