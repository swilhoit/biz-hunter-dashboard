    ...
    <div className="col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
      <div className="flex flex-col h-full p-5">
                ...
                props.members.map(member => {
                  return (
                    <Link key={member.name} className="block" to={member.link}>
                      <img className="rounded-full border-2 border-white dark:border-gray-800 box-content" src={member.image} width="28" height="28" alt={member.name} />
                    </Link>
                  )
                })
              }
        ...
        <div className="grow mt-2">
          <Link className="inline-flex text-gray-800 dark:text-gray-100 hover:text-gray-900 dark:hover:text-white mb-1" to={props.link}>
            <h2 className="text-xl leading-snug font-semibold">{props.title}</h2>
          </Link>
          <div className="text-sm">{props.content}</div>
        </div>
        <footer className="mt-5">
          <div className="text-sm font-medium text-gray-500 mb-2">{props.dates.from} <span className="text-gray-400 dark:text-gray-600">-&gt;</span> {props.dates.to}</div>
          <div className="flex justify-between items-center">
            ...
            <div>
              <Link className="text-sm font-medium text-violet-500 hover:text-violet-600 dark:hover:text-violet-400" to={props.link}>View -&gt;</Link>
            </div>
          </div>
        </footer>
      </div>
