...
-- This version handles existing user_id constraints and missing columns
...
-- 4. Migrate brands data with proper user_id handling
DO $$
DECLARE
...
BEGIN
  ...
  -- Get a default user_id (the first user in the system)
  SELECT id INTO default_user_id FROM auth.users LIMIT 1;

  -- Handle different scenarios
  IF user_id_exists AND NOT user_id_nullable THEN
    -- user_id exists and is NOT NULL, we need to provide it
    
    -- Check if deals table has user_id column
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'deals' AND column_name = 'user_id') THEN
      -- Use user_id from deals
      INSERT INTO brands (name, category, subcategory, user_id)
      SELECT DISTINCT 
        d.business_name as name,
        d.amazon_category as category,
        d.amazon_subcategory as subcategory,
        COALESCE(d.user_id, default_user_id) as user_id
      FROM deals d
    ...
    ELSE
      -- No user_id in deals, use default
      INSERT INTO brands (name, category, subcategory, user_id)
      SELECT DISTINCT 
        d.business_name as name,
        d.amazon_category as category,
  ...
  ELSE
    -- user_id doesn't exist or is nullable, proceed with simple insert
...
-- 11. Create updated brand_keyword_aggregate view that handles both old and new structure
...
BEGIN
  ...
  SELECT COUNT(*) INTO deal_count FROM deals WHERE brand_id IS NOT NULL;
  
  RAISE NOTICE 'Migration complete: % brands created, % deals updated with brand references', brand_count, deal_count;
END $$;