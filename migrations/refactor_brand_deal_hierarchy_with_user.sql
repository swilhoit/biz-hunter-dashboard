...
-- This version handles existing user_id constraints
...
-- 4. Check if user_id column exists in brands table
DO $$
DECLARE
...
BEGIN
  ...
  IF user_id_exists AND NOT user_id_nullable THEN
    -- user_id exists and is NOT NULL, we need to provide it when inserting
    -- Get the created_by user from deals or use a default user
    
    -- First, try to insert brands with user_id from their associated deals
    ...
    -- For deals without created_by, use the first available user_id
    INSERT INTO brands (name, category, subcategory, user_id)
    SELECT DISTINCT 
      d.business_name as name,
      d.amazon_category as category,
